...
class Kaixo(resource.Resource):
    async def render_get(self, request):
        return aiocoap.Message(payload="Kaixoooooo".encode('utf8'))

    async def render_post(self, request):
        payload = request.payload.decode('utf-8')
        print(f"POST request received with payload: {payload}")
        # Puedes procesar la carga útil (payload) como desees aquí
        return aiocoap.Message(code=aiocoap.numbers.codes.CREATED, payload="POST received".encode('utf-8'))
...
async def main():
    ...
    root.add_resource(["temperature"], Kaixo())
